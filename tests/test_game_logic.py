...
def test_character_level_up(player):
...
def test_engine_movement(engine):
    # Normal move
...
def test_resolve_combat(engine):
...
def test_combat_trigger_on_move(engine):
...
def test_save_load(engine, tmp_path):
